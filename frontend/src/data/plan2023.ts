import type { Materia, Electiva } from '../types/plan';
...
export const MATERIAS_MAP: Record<number, Materia> = {};
MATERIAS_TRONCALES.forEach(m => {
  MATERIAS_MAP[m.id] = m;
});

export const ELECTIVAS_MAP: Record<number, Electiva> = {};
MATERIAS_ELECTIVAS.forEach(e => {
  ELECTIVAS_MAP[e.id] = e;
});
